...
int main(void)
{
    ...
    uint32_t no_of_key_presses = 0;
    char printed_no_of_key_presses[] = "  ";
    ...
    //LCD static content
    //--------------------------------    
    printS_5x7(0, 0, "                         "); // Max 25 5x8 characters
    printS_5x7(0, 0, "    Key Press Counter    ");
    printS_5x7(0,24, "No. of key presses: 0    ");
    //--------------------------------
    //LCD dynamic content
    //--------------------------------        
while(1){
    while (pressed_key==0) pressed_key = KeyPadScanning();
    
    no_of_key_presses ++; // increment key press count value
    
    sprintf(printed_no_of_key_presses,"%d",no_of_key_presses); // print this value to a string
    
    printS_5x7(100,24, printed_no_of_key_presses); // update the key press value
    
    CLK_SysTickDelay(BOUNCING_DLY); // to eliminate key press bouncing
    
    pressed_key=0; // reset to wait for next key press
    }
}
