...
int main(void)
{
	...
	uint8_t x_pos = 0, dx = 1;
	uint8_t arrow_speed = 0;
	...
	while (1) {
		...
		switch(key_pressed) {
			case 1:
				draw_LCD(DDRAM_BUFFER_01);
				for (i = 0; i < 2; i++) CLK_SysTickDelay(SYSTICK_DLAY_us);
				draw_LCD(DDRAM_BUFFER_02);
				for (i = 0; i < 2; i++) CLK_SysTickDelay(SYSTICK_DLAY_us);
				LCD_clear();
				break;
			
			case 2:
				draw_Pixel(10,10,1,0);
				draw_Pixel(10,11,1,0);
				draw_Pixel(11,10,0,0); //fgcolor=0 --> write 0 to this pixel
				draw_Pixel(11,11,1,1);
				for (i = 0; i < 2; i++) CLK_SysTickDelay(SYSTICK_DLAY_us);
				LCD_clear();
				break;
			
			case 3:
				for (x_pos = 24; x_pos <= 64; x_pos = x_pos + dx){
					draw_Bmp16x16(x_pos, 0, 1, 0, diamond_16x16);
					CLK_SysTickDelay(150000);
					draw_Bmp16x16(x_pos, 0, 0, 0, diamond_16x16);
				}
				break;
				
			case 4:
				draw_Bmp32x48(0, 8, 1, 0, archer_32x48);
				for (i = 0; i < 2; i++) CLK_SysTickDelay(SYSTICK_DLAY_us);
			
				draw_Bmp32x48(0, 8, 0, 0, archer_32x48);
				draw_Bmp32x48(0, 8, 1, 0, archer_release_32x48);
				arrow_speed = 4;
			
				for (x_pos = 32; x_pos <= 90; x_pos = x_pos + arrow_speed){
					draw_Bmp32x16(x_pos, 16, 1, 0, arrow_32x16);
					CLK_SysTickDelay(150000);
					draw_Bmp32x16(x_pos, 16, 0, 0, arrow_32x16);
				}
				draw_Bmp32x48(0, 8, 0, 0, archer_release_32x48);
				break;
				
			case 5:
				draw_Circle(64, 32, 10, 1, 0);
				for (i = 0; i < 2; i++) CLK_SysTickDelay(SYSTICK_DLAY_us);
				draw_Circle(64, 32, 10, 0, 0);
				break;
			
			case 6:
				fill_Rectangle(64, 32, 64+8, 32+8, 1, 0);
				for (i = 0; i < 2; i++) CLK_SysTickDelay(SYSTICK_DLAY_us);
				fill_Rectangle(64, 32, 64+8, 32+8, 0, 0);
				break;
